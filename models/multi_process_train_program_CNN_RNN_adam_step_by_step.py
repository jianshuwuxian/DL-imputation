...
def build_model(n_c_snp, n_c_ind, ind_n, lr):
    """
    构建网络
    :return:
    """
    ...
    #model.add(layers.Reshape((ind_n, dimension), input_shape=((ind_n, 1, dimension),)))
...
def main_func(snp_num, data_dir, train_file_num, n_cross_snp, n_cross_ind, learning_rate):
    """
    数据预处理
    """
    ...
    for snp in range(0, int(snp_num)):
        ...
        with timer():
            ...
            #with timer("构建模型"):
            model = build_model(n_cross_snp, n_cross_ind, ind_num, learning_rate)
...
if __name__ == '__main__':
    ...
    main_func(snp_num=snp_num, data_dir=data_dir, train_file_num=train_file_num, n_cross_snp=N_cross_snp, n_cross_ind=N_cross_ind, learning_rate=lr)
