...
def data_generator(geno_data, snp_index, ind_index, N1, N2, ind_num, snp_num, LDMatrix, GMatrix, single_snp_alleles_array, single_ind_gmatrix_data):
    """
    为每个位点生成序列 横纵两个方向信息
    :param geno_data:
    :param snp_index:
    ...
    ## 亲缘关系排序版本
    #sorted_indexes = np.argsort(single_ind_gmatrix_data)[-1-(2*N2):-1][::-1]
    #seq = single_snp_alleles_array[sorted_indexes]
    #g_seq = single_ind_gmatrix_data[sorted_indexes]
    #snp_direction_seq = np.append(seq, g_seq)
    
    # 随机选位置版本
    net_ind_num = len(single_snp_alleles_array)
    random_indexes = []
    while len(random_indexes) < 2*N2:
        r_n = random.sample(range(0, net_ind_num), 1)
        if (r_n not in random_indexes) and (r_n != ind_index):
            random_indexes.append(r_n)
    random_indexes = np.array(random_indexes)
    seq = single_snp_alleles_array[random_indexes]
    g_seq = single_ind_gmatrix_data[random_indexes]
    snp_direction_seq = np.append(seq, g_seq)
