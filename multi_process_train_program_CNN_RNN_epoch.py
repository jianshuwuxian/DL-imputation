...
def main_func(snp_num, data_dir, train_file_num, epoch, n_cross_snp, n_cross_ind):
    """
    数据预处理
    """
    ...
    for snp in range(0, int(snp_num)):
        ...
        with timer():
            ...
            history = model.fit(train_data, train_labels, epochs=epoch, batch_size=128, validation_split=0.15,
                                    verbose=False)
...
if __name__ == '__main__':
    ...
    main_func(snp_num=snp_num, data_dir=data_dir, train_file_num=train_file_num, epoch=epoches, n_cross_snp=N_cross_snp, n_cross_ind=N_cross_ind)
