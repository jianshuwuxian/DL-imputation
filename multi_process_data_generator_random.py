...
# 2020/2/5 22:48
#
from data_random import get_raw_data, imputed_file_writer, data_generator, data_generator_using_matrix, getLDMatrix, getGMatrix
import numpy as np
import time
...
# window_size = 2 * N
# N = 10
N_cross_snp = 0  # the number of different snp alleles besides the loci
N_cross_ind = 2  # the number of different ind alleles besides the loci
# N = 5
...
def main_func(data_dir, geno_file_num, matrix_info=False):
    """
    数据预处理
    """
    ...
    net_file_name = data_dir + "miss_raw_without_header.txt"

    ld_file = data_dir + "plink.ld"

    gmatrix_file = data_dir + "sub_gmatrix" + str(geno_file_num) + ".txt"

    net_gmatrix_file = data_dir + "gmatrix.txt"
    ...
    with open(net_gmatrix_file, "r") as f:
        cur = 0
        while cur < ind_num:
            gmatrix_datas.append(np.array(f.readline().strip('\n').split()))
            cur += 1
    gmatrix_datas_array = np.array(gmatrix_datas)
    ...
    if ((2 * N_cross_snp) <= snp_num - 1) and ((2 * N_cross_ind) <= ind_num - 1):
        ...
        with timer(name='generate data', level='important'):
            for snp_index in range(0, snp_num):
                ...
                # 获得某个snp所有ind的等位基因
                line = linecache.getline(net_file_name, snp_index+1)
                #with open(net_file_name, 'r') as f:
                #    line = f.readline()
                #    line_count = 0
                #    while line_count < snp_index:
                #        line = f.readline()
                #        line_count += 1

                single_snp_line = line.strip('\n').split('\t')[9:]

                single_snp_alleles = []
                for ssl in single_snp_line:
                    final_allele = sum(list(map(int, ssl.split("|"))))
                    single_snp_alleles.append(5 if final_allele >= 5 else final_allele)
                single_snp_alleles_array = np.array(single_snp_alleles)
                
                for ind_index in range(0, ind_num):
                    if not matrix_info:  # 判断是否使用矩阵形式的信息
                        # 利用横纵两个方向的信息
                        p_seq = data_generator(geno_data_with_miss_array_snps, snp_index, ind_index, N_cross_snp, N_cross_ind, ind_num, snp_num, LDMatrix, GMatrix, single_snp_alleles_array, gmatrix_datas_array[ind_index])
        ...
        with timer(name='write train data to files', level='important'):
            # train_seq文件
            with open(data_dir + "sub_train_seq" + str(geno_file_num) + ".txt", "w") as f1:
                lines = []
            ...
            # train_label_seq文件
            with open(data_dir + "sub_train_label_seq" + str(geno_file_num) + ".txt", "w") as f2:
                lines = []
            ...
            # test_seq文件
            with open(data_dir + "sub_test_seq" + str(geno_file_num) + ".txt", "w") as f3:
                lines = []
                for t_s in test_seq:
                    line = ""
                    for item in t_s:
...
if __name__ == '__main__':
    ...
    main_func(data_dir=data_dir, geno_file_num=geno_file_num, matrix_info=False)
