...
def main_func(snp_num, data_dir, train_file_num, batch_size, n_cross_snp, n_cross_ind):
    """
    数据预处理
    """
    ...
    for snp in range(0, int(snp_num)):
        ...
        with timer():
            ...
            history = model.fit(train_data, train_labels, epochs=num_epochs, batch_size=batch_size, validation_split=0.15,
                                    verbose=False)
...
if __name__ == '__main__':
    ...
    batch_sizes = int(sys.argv[4])
    N_cross_snp = int(sys.argv[5])
    N_cross_ind = int(sys.argv[6])
    print('This_is_file ' + train_file_num)
    main_func(snp_num=snp_num, data_dir=data_dir, train_file_num=train_file_num, batch_size=batch_sizes, n_cross_snp=N_cross_snp, n_cross_ind=N_cross_ind)
