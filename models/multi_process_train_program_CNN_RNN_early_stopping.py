...
def build_model(n_c_snp, n_c_ind, ind_n):
    """
    构建网络
    :return:
    """
    dimension = n_c_snp * 2 + n_c_ind * 4
    
    model = models.Sequential()
    model.add(layers.Conv1D(32, 3, activation='relu', input_shape=(dimension, 1)))
...
def load_data(train_data, train_label, test_data):
    m_d = np.reshape(train_data, (len(train_data), len(train_data[0]), 1))

    d = np.reshape(train_label, (len(train_label), 1))
    x = np.array(test_data)
    test_d = np.reshape(x, (len(test_data), len(test_data[0]), 1))
    return m_d, d, test_d
...
def main_func(snp_num, data_dir, train_file_num, n_cross_snp, n_cross_ind):
    """
    数据预处理
    """
    ...
    for snp in range(0, int(snp_num)):
        ...
        with timer():
            ...
            #with timer("构建模型"):
            model = build_model(n_cross_snp, n_cross_ind, ind_num)

            '''
            训练模型
            '''
            my_callbacks = [EarlyStopping(monitor='val_loss', patience=20, mode='min')]
            #with timer("训练模型", "important"):
            history = model.fit(train_data, train_labels, epochs=200, batch_size=32, validation_split=0.15,
                                    verbose=False, callbacks=my_callbacks)
...
if __name__ == '__main__':
    ...
    N_cross_snp = int(sys.argv[4])
    N_cross_ind = int(sys.argv[5])
    print('This_is_file ' + train_file_num)
    main_func(snp_num=snp_num, data_dir=data_dir, train_file_num=train_file_num, n_cross_snp=N_cross_snp, n_cross_ind=N_cross_ind)
